...
manual_gui_confirm_fallback_ready() {
	...
	mode="$(read_file mode)"
	[ "$mode" = "gui" ] || return 1
	age_seconds="$(save_ts_age_seconds)" || return 1
	awk -v age="$age_seconds" -v threshold="$MANUAL_CONFIRM_FALLBACK_SECONDS" 'BEGIN { exit !(age >= threshold) }'
}
...
preconfirm)
	...
	elif manual_gui_confirm_fallback_ready; then
		claim_send_state preconfirm_manual || exit 0
		play_feedback_sound "$DJI_PRECONFIRM_SOUND_NAME"
